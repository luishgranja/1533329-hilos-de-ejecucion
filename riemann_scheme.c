...
double base_length = 0.0; // Variable que tiene el ancho de cada rectangulo
double sumTotal = 0.0; // Variable que tendra la suma total
...
void* calcular(void *arg) {
    ...
    for(double i=loslimit; i < (id+1)*numRectxThread ; i++){
		double x = i * base_length + BASE;
		partialSums[id] += function(x) * base_length;
	}    
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < MAXTHREAD; i++) {
		...
		// - calcular
		// - taskids
		pthread_create(&ptarray[i],NULL,calcular,&taskids[i]);
	}
